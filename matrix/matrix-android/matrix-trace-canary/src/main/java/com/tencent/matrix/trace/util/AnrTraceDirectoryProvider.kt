 ...
 */
internal class AnrTraceDirectoryProvider constructor(
  context: Context,
...
//          contentText,
//          pendingIntent,
//          R.id.leak_canary_notification_write_permission,
//          LEAKCANARY_LOW,
//          //added by jiayuliang
//          false,
//          false
...
//        )
  ...
  private fun externalStorageDirectory(): File {
    ...
    return File(downloadsDirectory, "anrtrace-" + context.packageName)
  }
...
//          SharkLog.d { "Could not delete old hprof file ${hprofFiles[i].path}" }
  ...
  companion object {
    ...
    private const val HPROF_SUFFIX = ".txt"

    fun hprofDeleteReason(file: File): String {
      val path = file.absolutePath
      return when {
        filesDeletedTooOld.contains(path) -> "older than all other trace files"
        filesDeletedRemoveLeak.contains(path) -> "trace manually removed"
        else -> "unknown"
      }
    }
  }
